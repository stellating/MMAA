...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder, src_embed, tgt_embed, rm, rm_num_slots, rm_d_model):
    ...
    def forward(self, src, mesh, tgt, src_mask, tgt_mask):
        return self.decode(self.encode(src, src_mask), mesh, src_mask, tgt, tgt_mask)

    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def decode(self, hidden_states, mesh, src_mask, tgt, tgt_mask):
        memory = self.rm.init_memory(hidden_states.size(0)).to(hidden_states)
        memory = self.rm(self.tgt_embed(tgt), mesh, memory)
...
class ConditionalLayerNorm(nn.Module):
    def __init__(self, d_model, rm_num_slots, rm_d_model, eps=1e-6, drop_path=0.3):
    ...
    def forward(self, x, memory):
        ...
        delta_gamma = self.drop_path(self.mlp_gamma(memory))
        delta_beta = self.drop_path(self.mlp_beta(memory))
...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, num_slots, d_model, num_heads=1):
        ...
        self.attn1 = MultiHeadedAttention(num_heads, d_model)
        self.mlp = nn.Sequential(nn.Linear(self.d_model, self.d_model),
                                 nn.ReLU(),
                                 nn.Linear(self.d_model, self.d_model),
                                 nn.ReLU())
        self.mlp1 = nn.Sequential(nn.Linear(self.d_model, self.d_model),
                                  nn.ReLU(),
                                  nn.Linear(self.d_model, self.d_model),
                                  nn.ReLU())
        self.mlp_mesh = nn.Sequential(nn.Linear(2048, self.d_model),
                                      nn.ReLU(),
                                      nn.Linear(self.d_model, self.d_model),
                                      nn.ReLU())
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, tgt_vocab):
        ...
        model = Transformer(
            ...
            rm, self.rm_num_slots, self.rm_d_model)
    ...
    def __init__(self, args, tokenizer):
        ...
        self.IDX2CLS = self.tokenizer.get_IDX_CLS()

        tgt_vocab = self.vocab_size + 1

        self.embeded = Embeddings(self.att_feat_size, tgt_vocab)
    ...
    def _prepare_feature(self, fc_feat, image_feature, hidden_state, att_masks):
        prob = torch.softmax(fc_feat, dim=1)
        class_indices = prob.data.topk(20, dim=1)[1].tolist()
        probs = prob.data.topk(20, dim=1)[0]
        class_names = []
        for cls_idx in class_indices:
            cls_n = []
            for i in cls_idx:
                cls_n.extend(self.IDX2CLS[i].split('_'))
            cls = ' '.join(cls_n)
            cls = list(set(self.tokenizer(cls)[1:-2]))
            class_names.append(cls)

        reports_meshs = class_names
        targets_meshs = np.zeros((len(reports_meshs), self.args.num_class), dtype=int)
        for i, report_meshs in enumerate(reports_meshs):
            targets_meshs[i, :len(report_meshs)] = report_meshs
        meshes = torch.FloatTensor(targets_meshs).cuda()
        mesh = self.embeded(meshes.long())

        # att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)
        if self.args.input_features == 'image_feature':
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(image_feature, att_masks)
        elif self.args.input_features == 'hidden_state':
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(hidden_state, att_masks)
        elif self.args.input_features == 'mesh_feature':
            att_feats = self.multimodal_feat(image_feature, mesh)
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)
        elif self.args.input_features == 'multimodel_feature':
            att_feats = self.multimodal_feat(image_feature, hidden_state)
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)

        memory = self.model.encode(att_feats, att_masks)

        return fc_feat, att_feats, memory, mesh, att_masks
    ...
    def _forward(self, image_feature, hidden_state, fc_feature, seq, att_masks=None):
        prob = torch.softmax(fc_feature, dim=1)
        class_indices = prob.data.topk(20, dim=1)[1].tolist()
        probs = prob.data.topk(20, dim=1)[0]
        class_names = []
        for cls_idx in class_indices:
        ...
        mesh = self.embeded(meshes.long())
        if self.args.input_features == 'image_feature':
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(image_feature, att_masks, seq)
        elif self.args.input_features == 'hidden_state':
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(hidden_state, att_masks, seq)
        elif self.args.input_features == 'mesh_feature':
            att_feats = self.multimodal_feat(image_feature, mesh)
        ...
        out = self.model(att_feats, mesh, seq, att_masks, seq_mask)
        out = self.logit(out)

        outputs = F.log_softmax(out, dim=-1)
        return outputs

    def core(self, it, mesh, fc_feats_ph, att_feats_ph, memory, state, mask):

        if len(state) == 0:
            ys = it.unsqueeze(1)
        else:
            ys = torch.cat([state[0][0], it.unsqueeze(1)], dim=1)
        out = self.model.decode(memory, mesh, mask, ys, subsequent_mask(ys.size(1)).to(memory.device))
