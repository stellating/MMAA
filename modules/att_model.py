...
class AttModel(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        self.ln = nn.LayerNorm(self.att_feat_size,eps=1e-05)
    ...
    def multimodal_feat(self, att_feats, meshes):  # Concate multimodal features
        # print(att_feats.shape,meshes.shape)
    ...
    def get_logprobs_state(self, it, mesh, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(xt, mesh, fc_feats, att_feats, p_att_feats, state, att_masks)
    ...
    def _sample_beam(self, fc_feats, image_feature, hidden_state, att_masks=None, opt={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, mesh, p_att_masks = self._prepare_feature(fc_feats, image_feature, hidden_state, att_masks)
        ...
        logprobs, state = self.get_logprobs_state(it, mesh, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)

        mesh, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = utils.repeat_tensors(beam_size,
                                                                                  [mesh, p_fc_feats, p_att_feats,
                                                                                   pp_att_feats, p_att_masks]
                                                                                  )
        self.done_beams = self.beam_search(state, logprobs, mesh, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, opt=opt)
        ...
        # print(seq,seqLogprobs）
        return seq, seqLogprobs

    def _sample(self, fc_feats, image_feature, hidden_state, att_masks=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, image_feature, hidden_state, att_masks, opt)
        if group_size > 1:
            return self._diverse_sample(fc_feats, image_feature, hidden_state, att_masks, opt)

        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size * sample_n)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, image_feature, att_masks)
    ...
    def _diverse_sample(self, fc_feats, att_feats, mesh, att_masks=None, opt={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, mesh, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_masks)
        ...
        for tt in range(self.max_seq_length + group_size):
            for divm in range(group_size):
                ...
                if t >= 0 and t <= self.max_seq_length - 1:
                    ...
                    logprobs, state_table[divm] = self.get_logprobs_state(it, mesh, p_fc_feats, p_att_feats, pp_att_feats,
